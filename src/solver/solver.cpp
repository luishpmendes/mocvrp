...
bool Solver::update_best_individuals(
    ...
    for (const auto & new_individual : non_dominated_new_individuals) {
        ...
        for (auto it  = best_individuals.begin();
            ...
            if (Solution::dominates(new_individual.first,
                                    individual.first,
                                    senses)) {
...
std::ostream & operator <<(std::ostream & os, const Solver & solver) {
    os << "Instance: " << solver.instance.name << std::endl
       ...
       << "Number of customers: " << solver.instance.num_customers << std::endl
       << "Vehicles capacity: " << solver.instance.capacity << std::endl
