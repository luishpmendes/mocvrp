...
void NSGA3_Solver::solve() {
    ...
    // the snapshots remain under the control of the solver. The memory keeps
    // the ideal point and the extreme points of the normalizing hyperplane
    // across the evolutions, which is what Section IV-C of the original paper
    // describes and what a single generation at a time would otherwise lose.
