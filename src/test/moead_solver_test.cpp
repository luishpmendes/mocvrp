...
int main() {
    ...
    for (const std::string filename : {"instances/X/X-n101-k25.vrp",
        ...
        solver = mocvrp::MOEAD_Solver(instance);
        ...
        assert(fabs(solver.cr - 1.0) <
            std::numeric_limits<double>::epsilon());
        assert(fabs(solver.f - 0.5) <
            std::numeric_limits<double>::epsilon());
        assert(fabs(solver.eta_m - 20.0) <
            std::numeric_limits<double>::epsilon());
        assert(fabs(solver.realb - 0.9) <
            std::numeric_limits<double>::epsilon());
